...
async def purge(ctx, amount, after=None, before=None):
	await ctx.defer()
	await ctx.channel.purge(deletion_limit=amount, after=after.split("-")[1], before=before.split("-")[1])
...
@interactions.slash_command(
	name="allowsleep",
	description="PC Sleep deprivation",
	options=[
		interactions.SlashCommandOption(
			name="allow",
			description="Sleepotion",
			type=interactions.OptionType.BOOLEAN,
			required=True
			),
		interactions.SlashCommandOption(
			name="display_on",
			description="Display on or off",
			type=interactions.OptionType.BOOLEAN,
			required=True
		)
	]
)
async def allowsleep(ctx, allow: bool, display_on: bool):
	if str(ctx.channel.category.name) == pcuser:
		if allow:
			prevent_system_sleep(display_on=display_on)
		else:
			allow_system_sleep()
		await ctx.send(f"Sleep {allow} with display {display_on}")
